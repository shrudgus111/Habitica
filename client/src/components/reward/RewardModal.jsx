		...
		<>
			<Modal className={`FL_Column ${modal && "active"}`}>
				{modal && (
					<>
						<CurrentCoin />
						<div style={{ flex: 1 }} className="DefaultWidth G_PCC RewardSection">
							<img src={selectedReward.img} alt={selectedReward.title} className="icon" />
						...
						<RewardButton onClickModal={onClickModal} selectedReward={selectedReward.price} />
					</>
				)}
			</Modal>
			<ModalBack className={`modalBack ${modal ? "active" : ""}`} onClick={onClickModal}></ModalBack>
		</>
	);
};
