...
boardRouter.post('/review/write', (req, res)=>{
    const {writer, subject, content} = req.body.board
    const date = dayjs()
    db.query("INSERT INTO reviewtbl (subject, writer, date, hit, content) VALUES (?, ?, ?, ?, ?)", [subject, writer, date.format('YYYY-MM-DD'), 0, content], (err, result)=>{
         if (err) {
             throw err
         } else {
             res.send(result)
...
boardRouter.post("/review/modify", (req, res)=>{
    ...
    db.query("UPDATE reviewtbl SET subject=?, content=? WHERE reNo=?", [subject, content, reNo], (err, result)=>{
        if (err) {
            res.status(500).send("수정 실패")
        } else {
            res.send(result)
