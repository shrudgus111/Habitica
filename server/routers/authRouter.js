...
authRouter.post("/join", (req, res) => {
  const { userId, userPw, userIrum, handphone, zipCode, addr1, addr2 } =
    req.body.addMember;
  db.query(
    "INSERT INTO membertbl (userId, userPw, userIrum, handphone, zipCode, addr1, addr2) VALUES (?, ?, ?, ?, ?, ?, ?)",
    [userId, userPw, userIrum, handphone, zipCode, addr1, addr2],
    (err, result) => {
      if (err) {
        throw err;
      } else {
...
authRouter.post("/modify", (req, res) => {
  const { userNo, userPw, userIrum, handphone, zipCode, addr1, addr2 } =
    req.body.userInfo;
  db.query(
    "UPDATE membertbl SET userPw=?, userIrum=?, handphone=?, zipCode=?, addr1=?, addr2=? WHERE userNo=?",
    [userPw, userIrum, handphone, zipCode, addr1, addr2, userNo],
    (err, result) => {
      if (err) {
        throw err;
      } else {
